...
1. **`core`**: Contains the foundational data structures (`ImageBuffer`, `GrayscaleImage`, `RGBImage`, `ImageBatch`) and the central `WorkspaceRegistry`. It also includes the `PCLBridge` for PixInsight compatibility and `MathUtils` for core statistical/matrix solvers (Gaussian Elimination, Cholesky, affine transforms, and robust estimators).
...
- **Heavy Statistics Parallelization**: When computing complex O(N²) image statistics for normalization or scaling (like the Rousseeuw Sn estimator), these operations must be parallelized with OpenMP to prevent them from becoming severe bottlenecks across batches.
- **Chunked I/O Caching**: When operating on large images in chunked modes (like Stacking), FITS file I/O readers should be cached at the thread level. Repeatedly opening and closing dozens of FITS files per patch incurs massive file system overhead.
...
- **Centroid Pre-filtering**: Instead of running heavy 2D Gaussian Nelder-Mead fits on all raw local maxima (many of which are noise or hot pixels), the Adaptive method first applies a fast, sub-microsecond moment-based centroid fit to all candidates.
